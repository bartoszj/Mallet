...
class RwT_Data:
    def __init__(self, rwt_pointer, params):
        ...
        if Utilities.is_valid_pointer(rwt_pointer.GetValueAsUnsigned(), params.pointer_size, allow_tagged=0):
            ...
            self.valid = 0
        if self.valid:
            self.rot = self.valobj.CreateValueFromData("rot",
                                                       lldb.SBData.CreateDataFromUInt64Array(self.sys_params.endianness,
...
class Class_Data_V2:
    def __init__(self, isa_pointer, params):
        ...
        if (isa_pointer is not None) and (
            Utilities.is_valid_pointer(isa_pointer.GetValueAsUnsigned(), params.pointer_size, allow_tagged=0)):
            self.sys_params = params
            ...
            self.valid = 0
        if self.valid:
            self.rwt = self.valobj.CreateValueFromData("rwt",
                                                       lldb.SBData.CreateDataFromUInt64Array(self.sys_params.endianness,
                                                                                             self.sys_params.pointer_size,
                                                                                             [self.dataPointer]),
                                                       self.sys_params.types_cache.addr_ptr_type)
    ...
    def check_valid(self):
        ...
        self.cachePointer = Utilities.read_child_of(self.valobj, 2 * self.sys_params.pointer_size,
                                                    self.sys_params.types_cache.addr_ptr_type)
        ...
        if not (Utilities.is_allowed_pointer(self.cachePointer)):
            ...
            self.valid = 0
            return
        self.dataPointer = Utilities.read_child_of(self.valobj, 4 * self.sys_params.pointer_size,
                                                   self.sys_params.types_cache.addr_ptr_type)
        ...
        self.superclassIsaPointer = Utilities.read_child_of(self.valobj, 1 * self.sys_params.pointer_size,
                                                            self.sys_params.types_cache.addr_ptr_type)
        if not (Utilities.is_valid_pointer(self.superclassIsaPointer, self.sys_params.pointer_size, allow_tagged=0,
                                           allow_NULL=1)):
...
class Class_Data_V1:
    ...
    def check_valid(self):
        ...
        self.namePointer = Utilities.read_child_of(self.valobj, 2 * self.sys_params.pointer_size,
                                                   self.sys_params.types_cache.addr_ptr_type)
    ...
    def instance_size(self, align=0):
        ...
        if self.is_valid() == 0:
            return None
        if self.instanceSize is None:
            self.instanceSize = Utilities.read_child_of(self.valobj, 5 * self.sys_params.pointer_size,
                                                        self.sys_params.types_cache.addr_ptr_type)
        if align:
            unalign = self.instance_size(0)
            if self.sys_params.is_64_bit:
